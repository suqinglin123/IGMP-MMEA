...
class Runner:
    def __init__(self, args, writer=None, logger=None, rank=0):
        ...
        if self.args.only_test:
            ...
            if self.args.update:
                assert self.args.update_start < self.args.epoch
                stage_1 = self.args.epoch
            else:
                stage_1 = self.args.epoch
            self.optim_init(self.args, total_epoch=stage_1)
    ...
    def model_choise(self):
        assert self.args.model_name in ["EVA", "MCLEA", "MSNEA", "IGMP"]
        if self.args.model_name == "IGMP":
            self.model = IGMP(self.KGs, self.args)
    ...
    def run(self):
        ...
        with tqdm(total=self.args.epoch) as _tqdm:
            for i in range(self.args.epoch):
                ...
                # -------------------------------
                self.epoch = i
                if self.args.il and (self.epoch == self.args.il_start and self.stage == 0) or (
                        self.early_stop_count <= 0 and self.epoch <= self.args.il_start):
                ...
                if self.stage == 1 and (self.epoch + 1) % 30 == 0 and self.args.il:
                    self.il_for_ea()
                if self.epoch < 100:
                    if (self.epoch + 1) % 2 == 0 and self.args.update:
                        self.update_image()

                else:
                    if (self.epoch + 1) % 30 == 0 and self.args.update:
                        self.update_image()

                if self.stage == 1 and (self.epoch + 1) % (self.args.semi_learn_step * 10) == 0 and len(
                        self.new_links) != 0 and self.args.il:
                ...
                _tqdm.set_description(
                    f'Train | Ep [{self.epoch}/{self.args.epoch}] Step [{self.step}/{self.args.total_steps}] LR [{self.lr:.5f}] Loss {self.loss_log.get_loss():.5f} ')
    ...
    def il_for_ea(self):
        if self.args.model_name in ["IGMP"]:
            final_emb, weight_norm = self.model.joint_emb_generat()
        else:
            final_emb = self.model.joint_emb_generat()
        final_emb = F.normalize(final_emb)
        self.new_links = self.model.Iter_new_links_train_only(self.epoch, self.non_train["left"], final_emb,
                                                              self.non_train["right"], new_links=self.new_links)
        if (self.epoch + 1) % (self.args.semi_learn_step * 5) == 0:
            ...
            self.logger.info(
                f"[TRAIN ONLY] Using only training set for image feature updates to prevent test data leakage")
    ...
    def update_image(self):
        if self.args.model_name in ["IGMP"]:
            ...
            final_emb = self.model.joint_emb_generat()
        final_emb = F.normalize(final_emb)
        self.new_links = self.model.Iter_new_image_features(self.epoch, self.non_train["left"], final_emb,
                                                              self.non_train["right"], new_links=self.new_links)

    def il_for_data_ref(self):
        self.non_train["left"], self.non_train["right"], self.train_ill, self.new_links = self.model.data_refresh(
            self.logger, self.train_ill, self.test_ill_, self.non_train["left"], self.non_train["right"],
    ...
    def eval(self, last_epoch=False, save_name=""):
        ...
        self.model.eval()
        self._test(test_left, test_right, last_epoch=last_epoch, save_name=save_name, is_final_test=False)
    ...
    def test(self, save_name="", last_epoch=True):
        ...
        # 最终测试阶段进行图像更新
        self._test(test_left, test_right, last_epoch=last_epoch, save_name=save_name, is_final_test=True)

    def _test(self, test_left, test_right, last_epoch=False, save_name="", loss=None, is_final_test=False):
        with torch.no_grad():
            w_normalized = None
            if self.args.model_name in ["IGMP"]:
                final_emb, weight_norm = self.model.joint_emb_generat()
        ...
        for idx in range(test_left.shape[0]):
            ...
            if last_epoch:
                ...
                to_write.append([idx, rank, test_left_np[idx], test_right_np[idx], test_right_np[indices[0]],
                                 ...
                                 test_right_np[indices[2]], round(values[0].item(), 4), round(values[1].item(), 4),
        ...
        if last_epoch:
            ...
            with open(osp.join(save_pred_path,
                               f"{self.args.model_name}_{self.args.data_choice}_{self.args.data_split}_{self.args.data_rate}_ep{self.args.il_start}_pred.txt"),
                      "w") as f:
                wr = csv.writer(f, dialect='excel')
                wr.writerows(to_write)
            if w_normalized is not None:
                with open(osp.join(save_pred_path,
                                   f"{self.args.model_name}_{self.args.data_choice}_{self.args.data_split}_{self.args.data_rate}_ep{self.args.il_start}_wight.json"),
                          "w") as fp:
                    json.dump(w_normalized.cpu().tolist(), fp)
            if weight_norm is not None:
                wight_dic = {"all": weight_norm.cpu(), "left": weight_norm[test_left].cpu(),
                             "right": weight_norm[test_right].cpu()}
                with open(osp.join(save_pred_path,
                                   f"{self.args.model_name}_{self.args.data_choice}_{self.args.data_split}_{self.args.data_rate}_ep{self.args.il_start}_wight_dic.pkl"),
                          "wb") as fp:
    ...
    def _load_model(self, model, model_name=None):
        ...
        if 'Dist' in self.args.model_name:
            model.load_state_dict(
                {k.replace('module.', ''): v for k, v in torch.load(save_path, map_location=self.args.device).items()})
        else:
